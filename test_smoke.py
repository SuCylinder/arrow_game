...
N = main.N
CELL = main.CELL
GX, GY, GW = main.GRID_X, main.GRID_Y, main.GRID_W

# 各关参考通关顺序（都是头部坐标，与 main.py 关卡注释一致）
ORDERS = [
    [(4, 4), (4, 2), (0, 1), (2, 3), (1, 0)],
...
def empty_board():
    return [[main.EMPTY] * N for _ in range(N)]
...
for idx, order in enumerate(ORDERS):
    level_defs = main.LEVELS[idx]
    game.level_index = idx
    game.load_level()
    check(f"第{idx + 1}关载入后为 PLAY", game.state == main.STATE_PLAY)
    n_arrows = main.count_arrows(game.arrows)
    ...
    for step, (r, c) in enumerate(order):
        ...
        game.try_click(r, c)
        game.update(1.0)
    check(f"第{idx + 1}关按参考顺序清空", main.count_arrows(game.arrows) == 0)
    check(f"第{idx + 1}关进入 WIN", game.state == main.STATE_WIN)
...
# 开始界面 → 开始游戏
game.to_start()
check("返回开始界面", game.state == main.STATE_START)
start_btn = next(b for b in game.buttons if b.label == "开始游戏")
game.on_click(start_btn.rect.center)
check("点「开始游戏」进第 1 关",
      game.state == main.STATE_PLAY and game.level_index == 0)

# 重开本关按钮 / R 键
game.mistakes_left = 1
restart_btn = next(b for b in game.buttons if b.label == "重开本关")
game.on_click(restart_btn.rect.center)
check("点「重开本关」失误回满", game.mistakes_left == main.MISTAKES_PER_LEVEL)
game.mistakes_left = 1
key_event(pygame.K_r)
game.handle_events()
check("按 R 重开本关", game.mistakes_left == main.MISTAKES_PER_LEVEL)
key_event(pygame.K_ESCAPE)
game.handle_events()
check("按 Esc 回开始界面", game.state == main.STATE_START)
...
check("点身体格子不扣失误", game.mistakes_left == main.MISTAKES_PER_LEVEL)
check("点身体格子有提示", "头部" in game.message)

# 点头部正常飞出
game.try_click(3, 2)
check("点头部触发飞出动画", len(game.anims) == 1
      and isinstance(game.anims[0], main.FlyAnim))
game.update(1.0)
check("长出整支箭头的所有格都被清除", game.board[3][2] == main.EMPTY
      and game.board[4][2] == main.EMPTY)
check("箭头活状态置为 False", not arrows[0].alive)

# 别人的身体会挡路：长 2 朝上的身体 (4,2) 挡住 (4,4) 朝左的箭头
board, arrows = main.build_arrows([
    (main.UP, 2, 3, 2),        # 头 (3,2)，身体 (4,2)
    (main.LEFT, 1, 4, 4),      # 头 (4,4)，朝左，路径上会撞到 (4,2)
])
game.board, game.arrows = board, arrows
game.arrow_by_id = {a.id: a for a in arrows}
game.anims.clear()
...
board, arrows = main.build_arrows([
    (main.UP, 2, 3, 0),        # 头 (3,0)，身体 (4,0)
    (main.LEFT, 1, 4, 4),      # 朝左，路径 (4,3)(4,2)(4,1)(4,0)
])
game.board, game.arrows = board, arrows
game.arrow_by_id = {a.id: a for a in arrows}
game.anims.clear()
...
board, arrows = main.build_arrows([
    (main.UP, 2, 3, 1),        # 头 (3,1)，身体 (4,1)
    (main.LEFT, 1, 2, 4),      # 头 (2,4) 朝左，路径 (2,3)(2,2)(2,1)(2,0)，不经过身体
])
game.board, game.arrows = board, arrows
game.arrow_by_id = {a.id: a for a in arrows}
...
# 基础 can_fly 边界测试
board, arrows = main.build_arrows([
    (main.UP, 1, 0, 0), (main.RIGHT, 1, 0, 4),
    (main.LEFT, 1, 4, 0), (main.DOWN, 1, 4, 4),
])
game.board, game.arrows = board, arrows
game.arrow_by_id = {a.id: a for a in arrows}
check("四角朝外均可飞（边界不越界）",
...
board, arrows = main.build_arrows([
    (main.RIGHT, 1, 2, 0), (main.UP, 1, 2, 3),
])
game.board, game.arrows = board, arrows
game.arrow_by_id = {a.id: a for a in arrows}
...
board, arrows = main.build_arrows([(main.UP, 4, 0, 0)])
game.board, game.arrows = board, arrows
game.arrow_by_id = {a.id: a for a in arrows}
...
try:
    main.build_arrows([(main.DOWN, 4, 0, 0)])   # 身体向上越过顶边
    check("越界箭头定义抛 ValueError", False)
except ValueError:
    check("越界箭头定义抛 ValueError", True)
try:
    main.build_arrows([(main.UP, 3, 0, 0), (main.UP, 2, 2, 0)])  # (2,0) 重叠
    check("重叠箭头定义抛 ValueError", False)
except ValueError:
    check("重叠箭头定义抛 ValueError", True)
try:
    main.build_arrows([(main.UP, 5, 4, 4)])
...
for length in (1, 2, 3, 4):
    for r in range(0, N - length + 1):
        fly_cases.append((main.UP, length, r, 2))
    for r in range(length - 1, N):
        fly_cases.append((main.DOWN, length, r, 2))
    for c in range(0, N - length + 1):
        fly_cases.append((main.LEFT, length, 2, c))
    for c in range(length - 1, N):
        fly_cases.append((main.RIGHT, length, 2, c))
...
for (d, length, r, c) in fly_cases:
    board, arrows = main.build_arrows([(d, length, r, c)])
    game.board, game.arrows = board, arrows
    game.arrow_by_id = {a.id: a for a in arrows}
    game.anims.clear()
    game.state = main.STATE_PLAY
    ...
    game.try_click(r, c)
    anim = game.anims[0]
    colors = [main.ARROW_COLORS[d], main.darken(main.ARROW_COLORS[d])]
...
for r in range(N):
    for c in range(N):
        ...
        if arrow.length < 2:
            continue
        d = arrow.direction
        color = main.ARROW_COLORS[d]
        body_color = main.darken(color)
...
for arrow in game.arrows:
    ...
    check(f"({hr},{hc}) 头部格中心为头部色",
          near(center, main.ARROW_COLORS[arrow.direction]))
    head_checked += 1
...
def region_has_text(x0, y0, w, h):
    for x in range(x0, x0 + w, 3):
        for y in range(y0, y0 + h, 3):
            if not near(game.screen.get_at((x, y)), main.BG, 12):
                return True
    return False
...
check("提示信息已绘制", region_has_text(main.WIDTH // 2 - 160, 100, 320, 26))
check("底部快捷键提示已绘制",
      region_has_text(main.WIDTH // 2 - 120, 610, 240, 26))
for btn in game.buttons:
    ...
    check(f"按钮「{btn.label}」已绘制",
          any(near(probe, c, 60) for c in
              (main.BTN, main.BTN_HOVER, main.BTN_PRIMARY, main.BTN_PRIMARY_HOVER)))

for st, tag in [(main.STATE_LOSE, "失败"), (main.STATE_WIN, "通关")]:
    game.state = st
    game._build_buttons()
    game.draw()
    check(f"{tag}界面标题已绘制",
          region_has_text(main.WIDTH // 2 - 200, 220, 400, 60))


# ---------------- E. 关卡数据性质 ----------------
check("每关都是箭头定义列表且非空",
      all(isinstance(lv, list) and lv for lv in main.LEVELS))

for idx, defs in enumerate(main.LEVELS):
    board, arrows = main.build_arrows(defs)       # 越界/重叠会抛异常
    check(f"第{idx + 1}关定义合法（不越界不重叠）", True)
    check(f"第{idx + 1}关箭头支数在 5~8", 5 <= len(arrows) <= 8)
    lengths = [a.length for a in arrows]
    check(f"第{idx + 1}关长度都在 1~4", all(
        main.MIN_LENGTH <= x <= main.MAX_LENGTH for x in lengths))
    check(f"第{idx + 1}关全部箭头都有身体（长度≥2）",
          all(x >= 2 for x in lengths))
    check(f"第{idx + 1}关长度混合（至少 2 种长度）", len(set(lengths)) >= 2)
    # 每支箭头的所有格都指向它自己
    ...
    game.arrow_by_id = {}
    reverse_ok = True
    real_board, real_arrows = main.build_arrows(defs)
    ...
    # 开局存在可飞箭头（不会开局死局）
    game.board, game.arrows = main.build_arrows(defs)
...
check("每关失误次数为 3", main.MISTAKES_PER_LEVEL == 3)

for size, sample in [(60, "箭头消除"), (22, "第关剩余失误飞出去了重开本"),
                     (20, "开始游戏下一关回"), (18, "按")]:
    font = main.make_font(size)
...
game.load_level()
board, arrows = main.build_arrows([
    (main.RIGHT, 2, 2, 2),     # 头 (2,2)，身体 (2,1)
    (main.LEFT, 1, 2, 4),      # 挡在 (2,2) 右侧
])
game.board, game.arrows = board, arrows
game.arrow_by_id = {a.id: a for a in arrows}
game.anims.clear()
...
check("碰撞生成晃动动画",
      len(game.anims) == 1 and isinstance(game.anims[0], main.ShakeAnim))
check("碰撞后整支箭头仍在原地", game.board[2][2] != main.EMPTY
      and game.board[2][1] != main.EMPTY)
game.update(0.25)
check("晃动中途头部颜色变红",
      game.anims[0].head_color() != main.ARROW_COLORS[main.RIGHT])
check("晃动中途身体颜色跟随变红",
      game.anims[0].body_color()
      != main.darken(main.ARROW_COLORS[main.RIGHT]))
game.update(0.5)
check("晃动结束箭头不消失",
      game.board[2][2] != main.EMPTY and game.board[2][1] != main.EMPTY)
check("晃动结束仍在 PLAY", game.state == main.STATE_PLAY)
...
check("失误归零时停在 PLAY（等动画）",
      game.state == main.STATE_PLAY and game.mistakes_left == 0)
...
game.update(1.0)
check("晃动结束进入 LOSE", game.state == main.STATE_LOSE)
...
# 同一支箭头动画期间连点锁定
board, arrows = main.build_arrows([(main.RIGHT, 2, 2, 2)])
game.board, game.arrows = board, arrows
game.arrow_by_id = {a.id: a for a in arrows}
game.anims.clear()
game.try_click(2, 2)
...
def random_pos():
    ...
    return (rng.randrange(main.WIDTH), rng.randrange(main.HEIGHT))
...
for step in range(6000):
    ...
    if game.state == main.STATE_PLAY:
        if not (0 <= game.mistakes_left <= main.MISTAKES_PER_LEVEL):
    ...
    if game.state in (main.STATE_WIN, main.STATE_LOSE) and game.anims:
        violations.append((step, f"{game.state} 状态仍有动画"))
    valid_ids = {a.id for a in game.arrows}
    for row in game.board:
        for v in row:
            if v not in valid_ids and v != main.EMPTY:
...
for _ in range(20000):
    game.on_click(random_pos())
    game.update(0.5)
    if game.state in (main.STATE_WIN, main.STATE_LOSE):
        resolved = True
        break
...
live = created[0]
click_event((main.WIDTH // 2, 430 + 26))       # 开始游戏按钮
time.sleep(0.6)
check("真实入口：点「开始游戏」进入 PLAY",
      live.state == main.STATE_PLAY and live.level_index == 0)

click_event(cell_center(0, 0))                 # 第 1 关 (0,0) 头部
time.sleep(0.6)
check("真实入口：点箭头头部运行正常", t.is_alive() and not crashed)

click_event(cell_center(1, 0))                 # 第 1 关 (1,0) 长的头部（变成可飞）
time.sleep(0.6)
check("真实入口：点多格箭头头部运行正常", t.is_alive() and not crashed)

key_event(pygame.K_ESCAPE)
time.sleep(0.6)
check("真实入口：Esc 回开始界面", live.state == main.STATE_START)
