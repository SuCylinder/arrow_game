...
from arrow.core.logic import fly_distance
from arrow.core.utils import lerp_color


class FlyAnim:
    """滑出动画：单格箭头沿它的方向滑出棋盘（ease-out，先快后慢）。"""

    def __init__(self, arrow):
        self.arrow = arrow
        self.r, self.c = arrow.head
        self.direction = arrow.direction
        self.distance = fly_distance(arrow)  # 需要滑出的格数（含安全余量）
        self.elapsed = 0.0
        dr, dc = DIRS[arrow.direction]
        self.axis = (dc, dr)  # 像素偏移方向（x, y）
        self.duration = 0.18 + 0.04 * self.distance
